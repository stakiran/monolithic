...
- 一日の最初に、主に「今日の」次にやることリストをメンテする
- 一日の終わりに、主に「明日の」次にやることリストをメンテする
...
### レビュー方法
Inbox リストのレビューで行う処理については [Stuff Filtering](#Stuff-Filtering) を参照。
...
### 粒度
Context の粒度は、タグ名は粗く、説明は細かくするのが良い。

タグ名はシステムに入力する都合上、あまり長くできない(また長いと単純に見づらかったりして不便である)ので、なるべく短くする。そのタグが表す意味は説明でカバーする。ただし、いちいち Context を付与する際に Context リストを見るのは手間なので、どのタグがどんな意味であるかを暗記しておいた方が良いのは言うまでもない。
...
![gtd_cccontext.jpg](img/gtd_cccontext.jpg)
...
### 粒度
Calendar の粒度は特に問わない。予定はユーザーにとってクリティカルであり、また数も多くないため、粗い記述でも十分読み解ける。
...
### [運用] ...
★アナログとデジタル。GTD 本見ても一般的な整理術の話になる。大掃除用リマインダー。
...
### 粒度
Ideal の粒度は細かいことが望ましい。

Ideal はユーザーが人生をかけて目指すゴールであるため、長期に渡って掲げ続けることになる。粒度が細かくなければゴールもあやふやになり、形骸化に繋がりかねない。

★粒度の意味がちょっとごっちゃになってきた。「どれだけ具体的か」と「どれだけ(タスクとして)細分化できているか」の二つがある。今はほぼ前者(前者に加えて Readable_や Understandable も含む)。でも後者の意味合いを想定する人もいるかも、だとしたらピンと来ない説明になっちゃう。 ……あー、でも後者かも。粒度って「どこまで細かく分解してるか」がたぶん正しい。タスク管理でも粒度が細かい＝小さなタスクに分解できている、だし。……とすると、粒度って言葉の見直しが必要か。具体と抽象？なんか違う。GTD 本は……粒度は全然取り扱ってないからなぁ。 → うん、修正しよう。こうしたい。「Ideal の粒度は粗い。1 つの Ideal に複数の Milestone がぶら下がるようにする」
...
- [タスク管理におけるコンテキストとは何かを調べてみた - タスク管理に恋してる](https://ilovetaskmanagement.hatenablog.com/entry/2018/09/21/192652)
