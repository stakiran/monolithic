...
連絡待ちとは「他者による何らかのアクション(反応や報告など)を経ないと先に進めない」であり、Waiting リストは連絡待ちを一箇所にまとめて俯瞰するために用いる。

### リストの複雑性
Waiting リストの複雑制はまずまずである。
...
### 粒度
Waiting リストに書く項目の粒度は任意である。ユーザー自身が各々どんな連絡待ちであるかを把握できればそれで良い。また、また、Waiting リストは高頻度でレビューを行うがゆえに、ユーザーは関連するコンテキスト(状況や文脈)を忘れにくいので、あまり細かくしなくても十分理解できることが多い。

### [運用] ★「waitingを確認する @2」と「～～の件を確認する @2」の違い
前者が望ましい。
確認に時間がかかる場合は後者で別タスクに切り出す。
的な説明。
