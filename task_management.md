...
### コンテキスト
★タスクが属するコンテキスト（文脈）です。「このタスクはこんな文脈下にありますよ」を示します。文脈というと難しい言葉ですが、要するに付加情報です。たとえば「場所」「状況」「使用道具」などです。提唱元はおそらく [GTD](https://en.wikipedia.org/wiki/Getting_Things_Done) と思います。

ツールでの命名例:

- Context(todo.txt, toodledo)
- Mode(TaskChute) ★モードは別枠にした方がよくない？
