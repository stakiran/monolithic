...
## 各データについて
★今のところはここに書いてる。長くなったら「# タスク管理データ」として独立させる。
...
コンテナには明示的コンテナと暗黙的コンテナがある。明示的コンテナとはカテゴリー、フォルダ、プロジェクトなどシステムによって明示的に提供されるコンテナ。暗黙的コンテナとは明示的コンテナを利用せずに実現された(無意識に実現されたものも含む)コンテナで、たとえば TODO リストには必ず「TODO を束ねたリストという構造の暗黙的コンテナ」が一つ存在する。コンテナという言葉が何を指しているか(明示的コンテナか、暗黙的コンテナか、両方か)は文脈次第であるため、注意深く読み解く必要がある。

明示的コンテナには明示的コンテナを含めることもできる(コンテナのネストという)が、通常これが実装されることはない。これはタスク管理が「多階層の分類」よりも「フラットなタグ付け」をもって管理されるのが主流だからである。仮に多階層が必要な場合でも、コンテナではなく関係パラメーターやリスト(つまり暗黙的コンテナ)などを用いる。
...
コンテナもパラメーターを持つ。
★観点、open/close(何か一言でサクっと定義したい。蓋ということでlidかなぁ？)。
★明示的パラメーターと暗黙的パラメーター。明示的はシステム側が定義するパラメーター。設定すればそのとおりに働く。暗黙的パラメーターはコンテナという概念に備わる性質。open/closedはこれ。closed listだと決めたからといって実際に追加編集が行えなくなるわけではなく、これはあくまでユーザーが「このコンテナ(リスト)には何も追加編集しない」と決めただけである。
★ここで紹介する暗黙的パラメーターは「このような性質を付与すると管理が捗る」という、よく知られた例である。
...
タスク管理ツールは、ユーザーが効率的かつ快適にタスク管理を行えるよう設定項目を設けており、ユーザーは好みの設定に変えることができる。
...
# ●misc
copy and away やりかけのタスクをその場で複製してから、割り込みを対応すること。割り込みを記録できる。片方はそのまま終了して、もう片方は（もし開始状態を扱うシステムなら）開始状態を解除しておいて、割り込みの対応をすること。
