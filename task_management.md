...
## 表現方式
★シームレス化どうかがデータ形式。で、それをどう表示するかが表示形式？表示形式の場合、たとえばtaskchuteでもモーダルダイアログで表示しちゃえばイシュー方式になる。 / 
★シームレスエディタブル名前が微妙。シームレス形式。一画面完結。直接編集……1 一覧から直接いじる、2 直接いじりもあるし別画面もある、3 別画面のみ、3 シームレス、2 ハーフシーム、3 シーム
...
★1 グルーピングは属性にすべき？属性はもうちょっとシンプルにできそうな気がする
★2 リッチ系はどうする？ファイル添付、チェックリスト、コメント……。イシュー型固有のプロパティ、でくくれそうな気はするが。
★3 階層系は親タスク子タスクのみ。ポインターは階層じゃないよな。関連？
★4 イシュー型の属性は特殊だし多すぎるので切り離した方がいい
...
★ふと思った タスク管理は時刻的管理と日付的管理に二分できるのでは？
...
★ライトなタスク管理。ヘビーなタスク管理。良いネーミングかも。taskuma 等で一日分ガッツリ管理するのがヘビー。
...
★カテゴリ系はどうする？プロジェクトはカテゴリとも言えそうだが。カテゴリとタグの違い？「分類項目」とかがわかりやすいか？
