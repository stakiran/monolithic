...
★特徴を端的に表現したい。ICV
ToM   I       忘れないために書き並べる。
TaM   I=C>V   個人的効率化。ブレイクダウン。ストイック。
TTAM  V>C     進捗管理。誰が何をしているか。どこまで進んでいるか。全体的にはどうか。
...
具体例もほしいか？
例: タスク管理本を執筆する
ToM  「タスク管理本を執筆する」一つとか。「2/14までに目次レビュー」「5/1デッドライン」こんなレベル。いつ何するかは全部自分の頭。最低限執筆や関連するイベントを逃さないように。
TaM  「タイトルをブレストする 15分で」「目次案をつくってみる 30分で」「目次案について見てもらいたいのでAさんにアポを取る 20分」 ← このレベルで洗い出して、ずらりと並べて、一つずつ消化していく。パズル。ゲーム。戦略。
TTaM  「タスク管理ツールの具体例を調べる @Aさん」 「一章を執筆する @Aさん @Bさん」「告知用ウェブサイトをつくる @Cさん」 ← こんな感じ。必要な作業は洗い出して割り当てる。が、各作業として具体的に何やるかまだは管理しない(各担当者に任せる。管理上は進捗がわかればそれでいい)
