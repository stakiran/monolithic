...
コンテナはタスクではない。タスク自身が分類機構を持つ場合、それはコンテナではなく関係パラメーターを持っているという。コンテナはファイルフォルダでいうフォルダのようなもので、コンテナそのものにタスクの機能はない。

コンテナにはコンテナを含めることもできる(コンテナのネストという)が、通常これが実装されることはない。これはタスク管理が「多階層の分類」よりも「フラットなタグ付け」をもって管理されるのが主流だからである。仮に多階層が必要な場合でも、コンテナではなく関係パラメーターや箇条書きなどを用いる。
...
## タスク管理の目的
タスク管理の目的を「何かを管理するため」と定めたとする。この「何か」が何であるかは(一般的にある手段の目的が人それぞれであるように)人それぞれだが、以下に大別できる。
...
- 行動を管理したい
  ...
  - 行動を決めるための行動(判断や選択など。メタ行動という)を省きたい
- 進捗を管理したい
...
```
[         Goal            ]   ★ ここも名前がほしい
[    XXXX   Management    ]   ★ XXXX Layer みたいな名前？
[    Task   Management    ]   ★
[Task][Task][Task]...[Task]   ★
```
...
これに近づくためには XXXX を管理しなければならない。

XXXX を管理するための方法は多数存在する。その一つはタスク管理。

タスク管理では「やること」をタスクと捉え、このタスクという単位をシステマチックに扱うことで「やること」の掃討を目指す。つまり XXXX の構成要素を「やること」だと解釈して、これを管理しようというアプローチ。

★タスク管理以外には何がある？「委任」とか？金持ちが家政婦雇うとか。男は仕事女は家事の男（家事部分を委任している）とか？
...
### テストデータ
テストデータ(Test-data)とはツール上で何らかの試行を行うためにつくったタスクデータ。
...
★元ネタマナーニャ。参考文献に書きたい。書き方も決めてしまおうか。ページ数まで記すのはだるいから書名だけでいいかなあ……？
...
- アーカイブ
  - 不要なタスクデータだが削除したくない or (開発者側の都合だが)削除すると負担が高いので隔離だけしたい場合、当該データに「削除済」という意味を付与する
...
## 移動
コンテナのみの設定を変更してタスクの所属先を変えること。タスク自体は何もいじらない。タスクをいじる場合は編集の範疇。
...
# ●misc
copy and away やりかけのタスクデータをその場で複製してから、割り込みを対応すること。割り込みを記録できる。片方はそのまま終了して、もう片方は（もし開始状態を扱うシステムなら）開始状態を解除しておいて、割り込みの対応をすること。

デイリータスクリスト マスタータスクリスト xxxxリストは用語としてどうしよう。xxxxだけ扱ってリストは付記して簡単に書くだけ？ ← オープンリストとクローズリストとかはもうリストだよなあぁ。あるいはオープンタスクという用語をつくってその中で説明するか、or オープンクローズに限ればタスクの項で性質として……いや性質じゃないかそれはリストというウワモノの性質。 / リスト……コンテナの一種として取り上げるか？

コンテナはリスト以外になるがある？リスト……一行一タスクで並べる。カラム……一カード一タスクで並べる。縦に並べる。ボード……カラムを横に並べる。
タスクリストとタスクセット。情報的データ構造でいうと。list tree set 他にある？stack queue。グラフ。連想配列。
...
skippable routine task。moody(ムラがある、気分屋の、不機嫌な) task というネーミングも良いかも、まあニュアンスはネガティブメインだけど、タスクこなさないのもネガティブな時が多いし……。

すぐに終わるタスク。GTD でいう 2 分のやつとか。
...
リマインド、リリマインド ← この辺どう扱う？
強い弱い後回し、受動的能動的割り込み ← この辺もどう扱う？
タスク管理を支援する周辺概念周辺システム。ここも扱っちゃう？キリなくない？イベントを扱うカレンダーについて詳しく取り上げたりするの？違うくない？
...
もう一つ、ジレンマじゃないけど「タスクに書いてあるとおりに従わない」という心理についても言及したい。ユーザーの不完全性。ユーザーサボタージュ。
……心理学の領分かしら。佐々木さん詳しそう。既にまとめてるかも。
...
転送。誘導。別のタスクリストを見るよう記述で指示させること。ツールによっては書いたリンクを開いたりといった支援機能もある？

コンテナとリストの違い。コンテナはfolder/projectのようにタスクという単位を入れるもの。中の表現形式は問うてない。一方リストはタスクの表現形式を問うたもの。コンテナの表現形式がリストだったりカードだったりする。「タスクリスト」←タスクが箇条書きで並んでいるもの。ここにコンテナが関与しているかどうかはわからない。単にコンテナのないシステムにおいてタスクが並んでいるだけかもしれないし、folderというコンテナがあってその中にタスクが含まれておりかつ表現方法として箇条書きを用いているかもしれない。 ← この辺、もうちょっとわかりやすく、かつ端的に示せないかな？

モチベーション、コンディション。タスクがあっても実行するとは限らないというか、実行者たる人間自身が不安定という側面をどうするかという話。

やらないといけない/やるべき/やった方がいい/やらなくてもいい。やりたい/やりたくない。8個？全部主観的。
