...
- 細かい(fine) - 粗い(coarse)
- 具体的(concrete) - 抽象的(abstract)
- 理解しやすい(readable) - 理解しにくい(non-readable)
...
細かいタスク(Fine Task)とは FC 境界で Fine 側に振れるタスクのこと。粗いタスク(Coarse Task)とは FC 境界で Coarse 側に振れるタスクのこと。

### 細かい立場と粗い立場
タスク管理において細かいタスクを主に扱うことを細かい立場という。

粗いタスクを主に扱うことを粗い立場という。

細かいタスクと粗いタスクを使い分けることを粒度中立または中立という。

細かい立場のユーザーはしばしば日常生活の(数分以内に終わる)雑事も管理する。たとえば「トイレ」「歯磨き」「持ち物確認」といったレベルでタスクを洗い出している。一日のタスク数が数十以上に及ぶことも珍しくない。この性質上、複雑な TaM ツールを利用していることが多い。
...
中立という立場については、認めない見方もある。この見方では「タスクは細かい粒度で管理するのが望ましい」「しかし手間暇や状況から非現実的なときは、やむを得ず粗い粒度で管理する」と考える。つまり立場として細かい立場になる、という主張である。

### 粒度フリーと粒度バインド
粒度フリー(Granularity Free)とは、粒度の大小で「タスクかどうかの判断」をしないこと、あるいはそのような立場を指す。逆に粒度フリーでないことを粒度バインド(Granularity Bind)という。

細かいタスクと粗いタスクとではタスクの性質がまるで異なるため、通常は粒度バインドであるべき。しかしながら、意図的に粒度フリーを取ることもある。
...
ちなみに、タスク管理について無知な者は基本的に粒度フリーである。これを無自覚の粒度フリー(IGF - Insensible Granularity Free)という。IGF な者と議論する場合は、細かいタスクの概念は通じないことが多いため、明示的に認識を共有する必要がある。
