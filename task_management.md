...
★隠しパラメーターの解説も必要かも。ロックや凍結は readonly パラメーターをいじると言えるが、これはユーザーには見えてない。少なくとも edit のエイリアスではない。

★ロックと凍結の違い。凍結は解凍しない限り一切 edit できない。ロックは管理者のみコメントを追加できるなど完全ではない。またロックは github issue などリッチなパラメーターを持つタスク(BTS)がサポートする。
