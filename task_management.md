...
一般的にツールにはあるタスク A に関する情報をすべて収容する余裕がない。また記述を工夫しても「タスク A の遂行に必要なすべての情報に効率的にアクセスできない」ことがある。このような場合の対処として、ツール上で記述量を(効率的にアクセスできる程度に)増やすことが考えられるが、スマートではない。そこで別アプローチとして、タスク A には「別の場所に保存してある情報 B を見ろ」という命令だけを記述していき、B 側で必要な情報を用意しておく、というものがある。これが転送タスクである。

★例として、記述中に記載したファイルパスを開く機能や、関連するファイルパスを記述できる属性を導入したツールなどがある。taskchute そういえばあったよな。
...
すぐに終わるタスク。GTD でいう 2 分のやつとか。soon/small task/short task/stone/小石……
