...
# レイヤーコントロール
pn では plane 上に phrase を配置していく。plane は言わば「レイヤー」であり、ある phrase 群が載った一枚の世界である。

このレイヤーはしばしば複数枚を運用することになる。使い分けや連携の要領は押さえておくべきであろう。本節にて解説する。

※データ単位としての正式名称は plane であるが、layer(レイヤー)の方がニュアンスとしてわかりやすいため、本節では後者の言葉を使う。

## 原則

### 1-layer 1-theme
一つのレイヤーは一つのテーマのみ扱うべきである。

というのも、無闇にレイヤーを増やすことは、フォルダを無闇に増やすような行動と等しく、「カテゴライズ」「分類」「整理」といった領域が絡んでくる。すでにその手のシステムや整理法が上手くいきづらいことは知られているが、pn も二の舞になってしまう。

### 1-main-layer と N-sub-layer
レイヤーはただ一つの main layer と、n つの sub layer という構成にするべきである。

また、レイヤー間のリンク構造は Star 型にするべきである。sub layer から sub layer へのリンクなどネストはしない。
...
バージョンレイヤー。main layerの過去バージョンを保存。

詳細レイヤー。main layerのもととなった調査、検討、試行内容を保存。過程の保存。
