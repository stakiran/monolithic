...
管理か……。具体的手法に手を出しちゃう？テキストファイルをフラットに配置する、ファイル名を yymmdd にする、catmemonote みたいなカテゴリー-ツリー-メモ構造、アウトライナーみたいに一ファイルで復数に区切る、twitterみたいにとにかく入れるだけ（時刻とかは自動で付与される）、grep、インターネットにメモする（googleでリーチできる）、メモの価値（評価機能入れる案と人工知能的に意味的解析する案）、ブックマークコメントでメモ、CLIのメモツール、タスク管理ツールにメモ、デイリーエリア（ある日のメモは全部この中にある、を日単位で）、
