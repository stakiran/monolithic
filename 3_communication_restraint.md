...
このような X を使うようにすれば、メンバーは必ず Scrapbox（というフロック型ツール）を経由することになる。安易にチャット上でコミュニケーションを進むのを防げるというわけだ。ちなみに、内密にやりとりしたい場合は、Scrapbox に「内密の件があるので電話ください」などと書けば良い。
...
究極的にはチーム内での取り決めの問題である。

既によく知られた方法が「形式（フォーマット）を定めて」「皆で守る」というものだ。これを **フォーマッティング** とでも呼ぶことにしよう。たとえば書籍は論文は、形式が統一されているため読みやすい。もちろん、形式があるとは形式を守ることであり、守るためのコスト（書くコストや整理するコスト）はかかる。

重要なのはトレードオフである。それも一度決めたら終わりではなく、状況に応じた調整は常に必要だ。フォーマッティングとは動的なものである。

さて、このフォーマッティングは、一体誰がどのように推進していけば良いのだろうか。あるいはどのようなツールや機能や方法論があれば回るだろうか。

★ちょっと言いたいことブレてる。口頭は揮発性（残らない）がゆえにテキトーにできていたが、フロックだと全部残るので整理や見せ方という概念が出てくるという話がしたいのだが。少人数ならテキトーでも通用するが、増えてくるとノイズも増えてきて無視できなくなる。コントロール役が必要になってくる。

★メンテナーだろうか。Stack Overflow や GitHub Issues もそうであるように、秩序を保つ番人がいて、そいつがあーだこーだ口を出す。是正する。
...
フロック型の先。フロー型に頼っているのを何とかする。集中検討会議。音声認識による入力アシスト。private areaの確保 etc。

拘束は緩いほどよいという原則。

拘束適応。拘束に適応してるとそれを脱するのに強烈な抵抗が感じる。
