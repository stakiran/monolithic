...
# LP ツールの具体例
本章では LP を実現するツールの具体例として拙作の [Tritask](https://tritask.github.io/tritask-web/) を取り上げる。

ドキュメントは一通り整えているので、適当に読んで理解したい読者の方は上記リンクよりウェブサイトにアクセスすると良い。

本章では Tritask を LP という観点で簡単に覗いてみる。

★どう説明していこうか
リストはこのようになっている。ずらり
リストはただのテキストファイルなので愛用のテキストエディタで書きやすい
デイリーリストの概念があり、今日やる行動はここに書く。それ以外は明日以降へ。
ルーチンタスクの概念があり、繰り返しを設定できる
並び順は各行単位の辞書順昇順である。sort するだけでうまい具合に並ぶ(今日やった、今日やる、今日やっている、明日以降やる、昨日以前やったの順)。そうなるようにフォーマットを設計している。
