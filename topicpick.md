...
# misc
inbox at 2021/11/05 18:13:26

> レイヤー。1、トピ間の自動リンク辿る。ルートメッシュ。2、トピを束ねるリンク集。/トピレイヤーとページレイヤー？トピはシステマチックにする、ページはスクボみたいに自由にする
> 
> ダイレクトページ。1トピ1ページだと直接ページが表示される
> 
> 部分キーワード自動リンク。これの作り方がポイントになりそう。される側は明示的に有効にするか。誰かが設定すればいい。関連の表示は動的にする
