...
### SWAP マトリックス
割り込みをどのように対処すればいいかを考える際には SWAP マトリックスを用いる。SWAP マトリックス(SWAP Matrix)とは、割り込みの能動性と後回しの強弱とで四象限をつくったマトリックスであり、割り込みの対処は能動性と強弱の組み合わせに応じて別々に検討するべきと主張したものである。名前の由来は Strong-Weak、Active-Passive の頭文字から来ている。
...
そこでタスク管理においては「資料支援」と題して、いかにして資料に素早く、また漏れなくリーチするかを模索する。

## ★misc
パスが示す先を開く系：クリッカブルリンク。ハイパーリンク。taskchuteもだったか
ワンプレイス原則 「ここに全部ある」 リーチが比較的単純になる
...
しかしながら、圧をまとったリストは（精度はさておき少なくともユーザー自身が必要と考えて洗い出したものであるから）紛うことなき現実であり、逃げる訳にはいかない。着実に一つずつ対処するか、タスクリストを見直して圧縮するかしかない。
