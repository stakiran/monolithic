...
GTD（気になることをリスト化＆後でレビューすることを担保することで気にならなくする → 目の前に集中できる、から集中支援の範疇かなぁ）
...
★集中の観点がブレてんぞ。タスク A があったとして、A の実行自体にいかに集中するかが集中支援のはず。インボックスとかは違うでしょ。結果的に A の実行に集中できるようになるかもしれないけど、そうじゃない。tms とは……あ、でも task management support、タスク管理支援だから、別にタスク A に直接関連してなくてもいい？……言葉で表わせ 「タスク A にどうやって取り組むか」とか ← そしてこれにそぐわない用語は阻害する。inbox も gtd もばいちゃ。/ 2つ支援がいる。「A に取り組むという前提で、A にいかに集中するか」を扱う集中管理と、直近取り組むべきタスク（dailytasklistなのかnexttasklistなのかはまちまちだが）たちにのみ集中させるやつ……いや後者は管理そのものやん。強いて言えば「主なタスク管理手法」のカテゴリだな。 / うん、これで。

★主なタスク管理手法：GTD（タスク管理以外も含んでるけど）、インボックス（溜めてゼロにするってだけだけど）、tritask、taskchute、……。あくまで概念やメソッドを取り上げるべき。ツールを取り上げないよう注意。
...
## ★misc
**タスクが絡む（タスクを表すページや画面から直接アクセスできる）支援だけ扱うこと（そうしないと整理術全般を扱うことになりかねない）**
...
これを遵守することでリストの読みやすさや扱いやすさを担保できる。しかし、この遵守には負担を要する(常に観点を意識してタスクを解釈・操作する必要があるため疲れる)ため、気を抜くと、すぐに観点に合わないタスクを追加してしまい、リストの基本が崩壊してしまい、収拾がつかなくなる。そうなると結果的にリスト管理に大きな手間がかかってしまい、破綻することさえある。

ベストプラクティスは、負担の高い遵守を最初から心がけることである。
